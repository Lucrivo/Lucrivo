...
select has_function(
  ...
  array[
    'uuid', 'boolean', 'bigint', 'bigint', 'bigint', 'bigint', 'bigint',
    'bigint', 'bigint', 'integer', 'boolean', 'bigint', 'integer', 'integer',
    'smallint', 'smallint', 'smallint', 'text', 'bigint', 'integer',
    'bigint', 'bigint', 'text', 'text', 'text', 'jsonb'
  ],
  'versioned Production report creation function exists'
);
select ok(
  not (
    select prosecdef
  ...
  'Production public report RPC is security invoker'
);
select ok(
  (
    select proconfig
...
select lives_ok(
  $$ select pg_temp.create_production_report(
    '70000000-0000-4000-8000-000000000060'
  ) $$,
  'a free user creates one Production report'
);
select throws_ok(
  $$ select pg_temp.create_production_report(
  ...
  'a second distinct Production report requires paid access'
);

reset role;

create function pg_temp.production_snapshot_v2(p_volume integer default null)
returns jsonb
language sql
immutable
as $$
  select jsonb_build_object(
    ...
    'results', jsonb_build_object(
      ...
      'realMarginBasisPoints', case when p_volume is null then null else 1200 end,
      'unitProfitCents', case when p_volume is null then null else 1200 end,
      'monthlyResultCents', case when p_volume is null then -300000 else 120000 end,
      'verdict', case when p_volume is null then 'operational_loss' else 'tight_margin' end,
...
create function pg_temp.create_production_report_v2(
  p_submission_id uuid,
  ...
  p_unit text default 'unit',
  p_report_snapshot jsonb default pg_temp.production_snapshot_v2()
)
returns bigint
language sql
as $$
  select public.create_production_diagnosis_report_v2(
    p_submission_id, true, 5000, 3000, 500, 1000, 500, 10000, 100000,
    p_monthly_sales_volume, true, 200000, 600, 200,
    p_schema_version, p_calculation_version, p_content_version, p_scenario,
    10000,
    case when p_monthly_sales_volume is null then null else 1200 end,
    case when p_monthly_sales_volume is null then null else 1200 end,
...
select ok(
  not exists (
    ...
    where p.oid = 'public.create_production_diagnosis_report_v2(uuid,boolean,bigint,bigint,bigint,bigint,bigint,bigint,bigint,integer,boolean,bigint,integer,integer,smallint,smallint,smallint,text,bigint,integer,bigint,bigint,text,text,text,jsonb)'::regprocedure
      and privilege.grantee = 0
      and privilege.privilege_type = 'EXECUTE'
  ),
  'PUBLIC cannot execute Production V2 creation'
);
select ok(
  not has_function_privilege(
    'anon',
  ...
  'anon cannot execute Production V2 creation'
);
select ok(
  not has_function_privilege(
    'service_role',
  ...
  'service role cannot execute Production V2 creation'
);

set local role authenticated;
select set_config('request.jwt.claim.sub', '', true);
select throws_ok(
  $$ select pg_temp.create_production_report_v2(
    '70000000-0000-4000-8000-000000000069'
  ) $$,
  '42501', 'authentication required', 'Production V2 requires a JWT subject'
);
select set_config(
  'request.jwt.claim.sub',
  '55555555-5555-4555-8555-555555555555',
...
select lives_ok(
  ...
  'Production V2 persists with omitted volume'
);
select results_eq(
  $$
    select p.monthly_sales_volume,
      d.report_snapshot #>> '{results,monthlySalesVolumeUsed}',
      d.report_snapshot #>> '{results,monthlyResultCents}', d.is_free_report
    from public.diagnoses d
    join public.production_diagnoses p on p.diagnosis_id = d.id
    where d.submission_id = '70000000-0000-4000-8000-000000000070'
  $$,
  $$ values (null::integer, '0'::text, '-300000'::text, false) $$,
  'Production V2 keeps null input, zero interpreted volume, monthly result, and paid flag'
);
select results_eq(
  $$ select pg_temp.create_production_report_v2('70000000-0000-4000-8000-000000000070') $$,
  $$ select id from public.diagnoses where submission_id = '70000000-0000-4000-8000-000000000070' $$,
  'Production V2 retry returns the same id'
);
select lives_ok(
  $$ select pg_temp.create_production_report_v2(
    p_submission_id => '70000000-0000-4000-8000-000000000071',
    p_monthly_sales_volume => 100,
    p_monthly_result_cents => 120000,
    p_verdict => 'tight_margin',
    p_priority => 'margin',
    p_report_snapshot => pg_temp.production_snapshot_v2(100)
  ) $$,
  'Production V2 persists with monthly volume'
);
select throws_ok(
  $$ select pg_temp.create_production_report_v2(
    '70000000-0000-4000-8000-000000000072', p_schema_version => 1::smallint
  ) $$,
  '22023', 'invalid production report snapshot', 'Production V2 rejects wrong schema version'
);
select throws_ok(
  $$ select pg_temp.create_production_report_v2(
    '70000000-0000-4000-8000-000000000073', p_calculation_version => 1::smallint
  ) $$,
  '22023', 'invalid production report snapshot', 'Production V2 rejects wrong calculation version'
);
select throws_ok(
  $$ select pg_temp.create_production_report_v2(
    '70000000-0000-4000-8000-000000000074', p_content_version => 2::smallint
  ) $$,
  '22023', 'invalid production report snapshot', 'Production V2 rejects wrong content version'
);
select throws_ok(
  $$ select pg_temp.create_production_report_v2(
    '70000000-0000-4000-8000-000000000075', p_scenario => 'resale'
  ) $$,
  '22023', 'invalid production report snapshot', 'Production V2 rejects wrong scenario'
);
select throws_ok(
  $$ select pg_temp.create_production_report_v2(
    '70000000-0000-4000-8000-000000000076', p_monthly_result_cents => -1
  ) $$,
  '22023', 'invalid production report snapshot', 'Production V2 rejects a tampered monthly result'
);
select throws_ok(
  $$ select pg_temp.create_production_report_v2(
    '70000000-0000-4000-8000-000000000077',
    p_report_snapshot => jsonb_set(pg_temp.production_snapshot_v2(), '{results,monthlySalesVolumeUsed}', '1')
  ) $$,
  '22023', 'invalid production report snapshot', 'Production V2 rejects a tampered interpreted volume'
);
select throws_ok(
  $$ select pg_temp.create_production_report_v2(
    '70000000-0000-4000-8000-000000000078',
    p_report_snapshot => jsonb_set(pg_temp.production_snapshot_v2(), '{category}', '"product"')
  ) $$,
  '22023', 'invalid production report snapshot', 'Production V2 rejects a tampered category'
);
select throws_ok(
  $$ select pg_temp.create_production_report_v2(
    '70000000-0000-4000-8000-000000000079', p_unit => 'hour'
  ) $$,
  '22023', 'invalid production report snapshot', 'Production V2 rejects a tampered unit'
);
select throws_ok(
  $$ select pg_temp.create_production_report_v2(
    '70000000-0000-4000-8000-000000000080', p_verdict => 'no_sales'
  ) $$,
  '22023', 'invalid production report snapshot', 'Production V2 rejects a tampered verdict'
);
select throws_ok(
  $$ select pg_temp.create_production_report_v2(
    '70000000-0000-4000-8000-000000000081', p_priority => 'price'
  ) $$,
  '22023', 'invalid production report snapshot', 'Production V2 rejects a tampered priority'
);
select throws_ok(
  $$ select pg_temp.create_production_report_v2(
    '70000000-0000-4000-8000-000000000082',
    p_report_snapshot => jsonb_set(pg_temp.production_snapshot_v2(), '{inputs,productionUnitCostCents}', '4999')
  ) $$,
  '22023', 'invalid production report snapshot', 'Production V2 rejects a tampered input'
);
select throws_ok(
  $$ select pg_temp.create_production_report_v2(
    '70000000-0000-4000-8000-000000000052'
  ...
  'Production V2 detects an incomplete existing submission'
);
select set_config(
  'request.jwt.claim.sub',
  '66666666-6666-4666-8666-666666666666',
