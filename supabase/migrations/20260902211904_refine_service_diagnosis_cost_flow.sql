...
add constraint service_diagnoses_work_period_check check (
  (
    work_hours_period = 'month'
    and work_period_minutes between 0 and 44640
    and monthly_work_minutes = work_period_minutes
  )
  or (
    work_hours_period = 'week'
    and work_period_minutes between 0 and 10080
    and monthly_work_minutes =
      ((work_period_minutes::bigint * 433 + 50) / 100)::integer
  )
  or (
    work_hours_period = 'day'
    and work_period_minutes between 0 and 1440
    and monthly_work_minutes =
      (
        (work_period_minutes::bigint * weekly_work_days * 433 + 50) / 100
      )::integer
  )
);

drop function public.create_service_diagnosis_report(
  uuid,
